#!/usr/bin/env bash
# Build, ad-hoc sign, package and Sparkle-sign one Tokenly release. Performs no upload or push;
...
NOTE="${2:-Tokenly ${VERSION}.}"
ROOT="$(cd "$(dirname "$0")/.." && pwd -P)"
OUT="$ROOT/build/out"
DD="$ROOT/Pulse/build/dd"
PROJ="$ROOT/Pulse/Pulse.xcodeproj"
APP="$DD/Build/Products/Release/Tokenly.app"
...
# Items published before the Pulse → Tokenly rename carry the old repo and the old DMG name, and
# the feed keeps them on purpose (dropping history would break Sparkle's skipped-version logic).
# They are the only enclosures allowed to deviate from the naming below — and only for check 2:
# the item this run publishes is still held to the exact current URL by check 1.
LEGACY_REPO_URL, LEGACY_PRODUCT = "https://github.com/itsSwanks/pulse", "Pulse"
...
for _, label, short, url in rows:
    want = "%s/releases/download/v%s/Tokenly-%s.dmg" % (repo_url, short, short)
    grandfathered = "%s/releases/download/v%s/%s-%s.dmg" % (LEGACY_REPO_URL, short, LEGACY_PRODUCT, short)
    if url not in (want, grandfathered):
...
want = prefix + "Tokenly-" + version + ".dmg"
if url != want:
...
ARCHS="$(lipo -archs "$APP/Contents/MacOS/Tokenly")"
...
cat > "$OUT/Tokenly-${VERSION}.html" <<HTML
...
<div class="n"><h2>Tokenly ${VERSION}</h2><p>${NOTE_HTML}</p></div>
HTML
...
Next steps (nothing below is done by this script):
  ...
  2. Create the GitHub release v$VERSION on that commit and upload $DMG:
       (web UI, or: gh release create v$VERSION $DMG --title "Tokenly $VERSION" --notes "$NOTE")
