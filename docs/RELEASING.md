...
Pulse is published anonymously: ad-hoc signed, no notarization, no Developer ID, no telemetry.
...
1. `build/release.sh <version> "<one-line note>"` — bumps `project.yml`, builds a universal Release,
   ad-hoc signs Sparkle's nested bundles and the app, packages `build/out/Pulse-<version>.dmg`
...
3. Create the GitHub release `v<version>` **on that commit** and upload `build/out/Pulse-<version>.dmg`
   (web UI, or `gh release create v<version> build/out/Pulse-<version>.dmg`).
...
2. **Scan the whole history.** The working tree can be clean while the history is not: an early
   ...
   anything — as it does for Pulse's development history — the public repository is cut as a single
...
3. **Cut and push the snapshot.** Cut it from the merged, tested `main` *after* the last change meant
   ...
   does not exist yet.
       ...
       git commit -m "Pulse <version>"
       git push -u origin public:main             # a. the snapshot becomes the public main

       # b. create the GitHub release v<version> on that commit and upload build/out/Pulse-<version>.dmg
...
as normal, and `private-history` can be deleted once nothing in it is still needed. Pulse's first
...
> **"Pulse" Not Opened**
> Apple could not verify "Pulse" is free of malware that may harm your Mac or compromise your privacy.
...
- `xattr -dr com.apple.quarantine /Applications/Pulse.app`, then open Pulse normally. Always works,
  no authentication.
- System Settings → Privacy & Security → **Security**, where a row appears after the refused launch:
  *"Pulse" was blocked to protect your Mac.* with an **Open Anyway** button. It asks to confirm
  ("Open "Pulse"? Apple is not able to verify that it is free from malware…") and then requires an
...
from — build both versions with it, then sign a local feed by hand in a staging directory of its own:
    ...
    mv build/out/Pulse-0.9.0.* /tmp/            # keep the installer, out of the next run's way
    build/release.sh 0.9.1 "Update test."
    mkdir -p /tmp/PulseFeed && cp build/out/Pulse-0.9.1.dmg build/out/Pulse-0.9.1.html /tmp/PulseFeed/
    . build/sparkle-tools.sh
    "$SPARKLE_BIN/generate_appcast" --embed-release-notes --ed-key-file build/.sparkle_ed_private_key \
      --download-url-prefix "http://localhost:8765/" --maximum-deltas 0 /tmp/PulseFeed
    ( cd /tmp/PulseFeed && python3 -m http.server 8765 )
...
Install `/tmp/Pulse-0.9.0.dmg` somewhere ad-hoc-signed and hit *Check now*: Sparkle should offer
0.9.1, install it and relaunch into it. Afterwards throw all of it away —
`git checkout -- Pulse appcast.xml`, re-run `xcodegen generate`, `rm -rf /tmp/PulseFeed /tmp/Pulse-0.9.0.*`,
...
turn Wi-Fi off for 6 minutes and confirm the rings show stale · delete `~/.codex/auth.json` →
disconnected copy, restore → live · Sparkle update from the previous release on an ad-hoc-signed
install · `build/check-anonymity.sh` clean on the .app and the DMG.
