...
- **Claude credentials are file-first.** An unexpired `~/.claude/.credentials.json` is returned without reading the Keychain at all; the Keychain is consulted only when the file is missing, undecodable, or expired. Only an explicit user refusal (Deny / cancel) latches the Keychain off for the life of the process so a polling app never re-prompts; transient failures — notably `errSecInteractionNotAllowed` around sleep/wake and the lock screen — are retried on the next poll, because latching on them disconnected Claude until relaunch after every Mac sleep.
