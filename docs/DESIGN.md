# Tokenly — AI usage edge dock for macOS · Design spec

The product name is **Tokenly** (rebranded 2026-08-31, bundle id `com.tokenly.mac`); internal
modules, types, folders and the Xcode target/scheme keep their original `Pulse` names, and the
sections below are left as they were written — read "Pulse" there as the product's former name.
...
3. ~~Dock is **always visible** by default; auto-hide is a setting.~~ Since v4: **auto-hide is on
   by default** — the dock rests as the glass handle and slides out on approach; always-visible is
   the setting.
...
- **Auto-hide** — toggle (default on since v4; §1 decision 3 superseded).
...
1. **No personal data in the artifact.** Bundle ID `com.pulsedock.mac` (now `com.tokenly.mac`). `NSHumanReadableCopyright` names no person. No author name in `Info.plist`, About panel, or source headers. Git commits are attributed to the maintainer's GitHub handle through its `users.noreply.github.com` address, configured per-repo — the handle is public by choice; a legal name, personal e-mail or timezone never is (commits are stamped UTC).
...
- **Keychain read never prompts.** The `Claude Code-credentials` item is read by running `/usr/bin/security find-generic-password -w` — the tool Claude Code itself creates and reads the item with, so it is on the item's access list. Calling the Security framework directly from the ad-hoc-signed app raised the macOS keychain password dialog instead, and because the app's identity is the cdhash of one build, *Always Allow* never outlived an update. A tool that does not return is abandoned after a timeout so a poll cannot wedge on an unanswered keychain-unlock dialog.

## 15. Amendments (2026-08-30)

Changes made during implementation that this design did not anticipate:

- **Claude credentials are file-first.** An unexpired `~/.claude/.credentials.json` is returned without reading the Keychain at all; the Keychain is consulted only when the file is missing, undecodable, or expired. On macOS Claude Code keeps the live token in the Keychain and writes the plaintext file only as a fallback, so an old file on disk is normally stale and the Keychain answers every poll. Every Keychain failure is retried on the next poll — there is no latch, because the `security`-tool read has no refusal to latch on.
...
## 18. UI v4 — Liquid Glass (2026-08-30)

Tokenly's three floating surfaces — the dock, the hover callout and the settings sheet — become
real Liquid Glass, the material macOS 26 uses for its own Dock, menus and toolbars: translucent,
refracting what is behind it, lit by a specular rim, adapting its tint to the desktop and to the
system Light/Dark setting. This supersedes the hand-painted tint-over-vibrancy glass of §16–§17.

### 18.1 Platform

- **macOS 26 (Tahoe) or later.** The glass is `.glassEffect` (SwiftUI) inside the existing
  non-activating panels; there is no look-alike for older systems. `LSMinimumSystemVersion` and
  the deployment target move to 26.0, so the update feed never offers this to a 0.1.0 install on
  macOS 14/15.
- **Adaptive appearance.** Nothing pins `.darkAqua` any more and no surface paints an opaque
  fill; text is `.primary`/`.secondary` (vibrant on glass). The dark-mode defect in the 0.1.0
  settings sheet — native segmented controls drawing system-light text onto a hard-coded black
  fill — disappears with the fill.
- **Accessibility.** Reduce Transparency frosts the glass and Increase Contrast sharpens edges
  automatically (system behaviour); Reduce Motion replaces every spring below with a 0.18 s
  ease-out (`Motion.reduced`).

### 18.2 One glass layer per surface

Glass is the floating control layer and is never stacked: each surface is exactly one
`.glassEffect(.regular, in: shape)`, and everything inside it is drawn with vibrant fills, never
with more glass.

| Surface | Glass shape | Inside |
|---|---|---|
| Dock | `DockShape` — Notch (concave flares from the screen edge) or **Pill** (the flare-less slab, formerly "Glass") | ring wells as `.primary.opacity(0.08)` discs with a hairline highlight; rings; vibrant labels; gear |
| Callout | `CalloutBubbleShape` — card **and tail** in one path, so the tail refracts too | header, bars (`.primary.opacity(0.12)` track + level colour), retry pills |
| Settings | `.rect(cornerRadius: 22)` | native macOS 26 controls: capsule segmented pickers, switches, bordered *Check now*; `.secondary` headers; red *Quit* |
| Handle | `EdgeSlabPath` capsule (9 pt) | glow dots |

"Inside" is literal: the dock's rings, labels and gear and the handle's dots are the *content* of
`.glassEffect`, not siblings stacked over a `Color.clear` slab. Nested, the framework resolves
`.primary`/`.secondary` against the tone the glass actually rendered; stacked, they resolved
against the window's appearance instead, so a window whose glass had gone dark while its content
stayed light drew near-black labels on dark glass (measured 1.5:1).

The dock and settings windows carry an AppKit window shadow exactly as the callout does (a
transparent `CalloutPositioner.shadowMargin` band that hit-testing ignores). The setting is a
*shape* choice — labelled **Dock shape**, Notch / Pill — while the type stays `DockStyle` and the
stored value `glass` is read as `pill`.

### 18.3 Dock size

A new **Dock size** setting — Small / Medium / Large, **Small by default** — scales the whole slab
and everything in it from one factor:

| | Small | Medium | Large |
|---|---|---|---|
| scale | 0.8 | 1.0 | 1.25 |
| slab width | 62 | 78 | 98 |
| ring ⌀ / stroke | 38 / 2.8 | 48 / 3.5 | 60 / 4.4 |
| percent label | 13 pt | 16 pt | 20 pt |

`DockMetrics(size:)` is the single source (`Metrics.*` keeps only the non-dock constants);
`DockLayout`, `DockShape`, `DockView` and `RingView` read it from the layout. Changing the size
morphs the dock on a settle spring: the window animates to its new frame and the content
animates its geometry on the same clock. The handle, callout and settings do not scale.

### 18.4 Motion

| Event | Animation |
|---|---|
| ring hover / press | lift to 1.08× / dip to 0.94× on `Motion.lift` (spring 0.35 s, bounce 0.35) |
| percent changes | digits roll (`contentTransition(.numericText)`) on `Motion.sweep`; the ring sweeps on the same smooth 0.7 s spring |
| callout appears | materializes out of the dock: glass `.materialize` transition + scale from 0.94 anchored at the tail, `Motion.materialize` (spring 0.4 s, bounce 0.2) |
| callout follows | window glides on `Motion.springCurve` (an overshooting cubic — `(0.34, 1.28, 0.64, 1)`) over `Motion.calloutFollow` |
| settings opens | same materialize, anchored on the gear side |
| dock slides / parks / resizes | window frame on `Motion.springCurve` over `Motion.dockSlide` (0.42 s); content geometry on `Motion.settle` (spring 0.45 s, bounce 0.15) |
| handle crossfades | alpha over `Motion.handleFade`, which *is* `Motion.dockSlide` — the two halves of the same hide/show land together (v3's 250 ms ended early) |
| segmented selection, switches | the system's own Liquid Glass morph and switch bounce |
| near-limit breathing | unchanged (2 s ease in/out) |

### 18.5 Colour

Ring and bar colours are unchanged in dark mode and become appearance-aware: the neon yellow
`E4FF1A` is `B8A400` in light mode so it survives a bright wallpaper; green, orange and red are
shared. Track, wells and pills are `.primary` at low opacity; secondary text is `.secondary`;
the disconnected dash is `.secondary.opacity(0.6)`. The former `Palette.panel`, `well`, `track`,
`gray`, `pillHover`, `glassTint`, `glassTail`, `glassBorder`, `glassHighlight`, `handleTint` and
`handleBorder` are removed.

### 18.6 What stays

Dock metrics at Medium (78 pt slab, 48 pt rings, 22 pt corners, 20 pt flares), callout width and
content, settings content and order, hover/auto-hide behaviour, alerts, the poll loop, PulseCore.

Known cosmetic limitation: a settings sheet left open across a *live* system Light/Dark flip can
lose the green tint on its segmented pickers — they draw grey until the sheet is closed and
reopened — because AppKit does not always re-tint an already-materialised `NSSegmentedControl`.
Everything else on the sheet, and every other surface, re-colours immediately.

## 19. Settings v5 — System-Settings density + Appearance (2026-08-31)

**Pixel source of truth:** `design/v4/Tokenly Settings Redesign.dc.html` (variants 1a dark / 1b light)
+ `design/v4/CHANGES.md` §7/§7b. The design's rgba color tables are its own CSS *fallback*; this
app keeps the real Liquid Glass sheet (§18) and adaptive `.primary`/`.secondary` structure, which
the design itself prefers ("native materials … so both themes come free").

### 19.1 Panel

One glass sheet, same materialize/dissolve lifecycle as §18.4, new geometry:

- Width **352 pt** (was 300), corner radius **16** (was 22), padding **14 / 14 / 12** (was 20).
- Content is three **inset grouped lists** (radius 10, adaptive fill `Color.primary.opacity(0.06)`,
  0.5 pt ring `Color.primary.opacity(0.07)`), separated by 0.5 pt hairlines
  (`Color.primary.opacity(0.08)`) inset to the row's leading padding.
- Typography: 13 pt row labels (−0.08 tracking), 11 pt secondary details, 11.5 pt footer copy.
  ~40 % shorter than v4's panel at the same content.

### 19.2 Header

App icon (the real `NSApp.applicationIconImage`, 19 pt, radius 5) + **Tokenly** 13 pt semibold +
version 11 pt secondary tabular, then a 20 pt circular close button (✕ 9 pt bold, fill
`primary.opacity(0.08)`, hover `0.16`; Esc still closes). No "Tokenly settings" title line.

### 19.3 Providers group

Single uppercase section header **PROVIDERS** (11 pt semibold, +0.5 tracking, secondary).
Rows are 38 pt: two-bar drag grip (11 × 1.5 pt, `primary.opacity(0.3)`, 2.5 pt apart) →
15 pt provider glyph (dimmed to 34 % when the provider is off) → name 13 pt → **live usage**
11 pt secondary tabular right-aligned → green switch (`.switch`, `.small`, `Palette.green`).

- Live usage string: enabled + snapshot → `"N%"` (session window, rounded); enabled but
  loading/disconnected → `"—"`; disabled → `"Off"`. The panel observes `UsageStore.statuses`
  directly, so the numbers move while the panel is open.
- **Drag to reorder** replaces the ▲/▼ buttons: the drag gesture lives on the grip only, so row
  clicks and switches stay unambiguous (the §18-era `List.onMove` failure cannot recur — there is
  no `List`). The lifted row follows the finger; the others make way with `Motion.settle`; drop
  commits the full order through the same `reorder(_:)` whole-array contract.
- Two constraints proven the hard way: the gesture measures in a **named coordinate space on the
  sheet**, never `.local` — the dragged row offsets itself by the reported translation, so a
  local-space reading subtracts the row's own movement and oscillates around half the finger's
  travel; and the click-away monitor **ignores a down inside the sheet** — the window server can
  hand a first-mouse (or synthesised) down on the not-yet-key panel of an inactive app to the
  global monitor even though the panel receives it too, which read as "away" and closed the panel
  mid-drag.

### 19.4 Options group

Five 40 pt rows, label left, stock segmented picker right — **untinted**: selection is the
system's neutral raised pill. Green now means *on* (switches) and ring semantics only.

| Row | Options | Notes |
|---|---|---|
| Appearance | Auto · Light · Dark | new — §19.6 |
| Alert threshold | 80% · 90% · 95% | unchanged semantics |
| Screen edge | Left · Right | unchanged |
| Dock style | Notch · Pill | copy "Dock style" (was "Dock shape") |
| Dock size | S · M · L | short labels; same `DockSize` cases |

### 19.5 Behavior group + footer

Three 40 pt rows with 11 pt secondary subtitles:

- **Auto-hide** — "Retract to a glass handle at the edge"
- **Launch at login** — "Start Tokenly when you sign in"; while macOS holds the registration for
  approval the subtitle swaps to "Approve Tokenly in System Settings → Login Items"
- **Automatic updates** — "Checked in the background once a day", with the small bordered
  **Check now** button between subtitle and switch (still disabled when the updater is
  unconfigured)

Below the groups, quiet About copy (11.5 pt secondary): *"Reads usage from your local Claude,
Codex and Gemini sign-ins. Talks only to those providers and to GitHub for updates. No
analytics."* Footer line: **View source on GitHub** (`Palette.link`, dynamic `#7CC6D8` dark /
`#0066CC` light) left; **Quit Tokenly** right — 11.5 pt secondary that turns `#FF6961` on a
`red.opacity(0.16)` rounded-6 pill on hover. No red-by-default quit, no bordered About box.

### 19.6 Appearance — Auto / Light / Dark

New preference `appearance` (`AppearanceMode: auto | light | dark`, default `auto`, invalid →
`auto`). Applied as one assignment — `NSApp.appearance = nil / .aqua / .darkAqua` — at launch and
on change. Because every surface already draws with Liquid Glass + adaptive `.primary`/
`.secondary`/dynamic `NSColor`s (§18.5), that single override themes the dock, callout, settings
panel and alert banner together; `auto` (appearance `nil`) tracks the system live, mid-flight.
Semantic ring colors are identical in both themes (`Palette.yellow` stays the only
per-appearance split). The design's §7b color table is the hand-rolled fallback and is
