...
1. **Edge peek (auto-hide mode):** resting state is a ~6px sliver at the edge showing only 3×22px color-coded bars (one per provider, semantic color). Cursor within ~44px of the edge slides the dock out (260ms, `cubic-bezier(0.32,0.72,0,1)`, translateX 72px). Retracts after a 400ms grace delay once the cursor leaves the dock region (>200px away), unless a callout is pinned or settings are open. Toggle between Auto-hide / Always visible.
