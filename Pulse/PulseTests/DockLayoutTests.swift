...
@Suite struct DockLayoutTests {
    ...
    @Test func metricsScaleTheWholeLayout() {
        let m = DockLayout(cellCount: 3, edge: .right, metrics: DockMetrics(size: .medium))
        let l = DockLayout(cellCount: 3, edge: .right, metrics: DockMetrics(size: .large))
        #expect(l.size.width == 98 && m.size.width == 78)
        #expect(l.size.height > m.size.height)
        #expect(l.flareInset == 25 && m.flareInset == 20)
        #expect(l.windowSize.height == l.size.height + 50)
        #expect(l.cellCenters[0].x == 49)
    }

    @Test func defaultMetricsAreMedium() {
        #expect(DockLayout(cellCount: 1, edge: .left).metrics == .medium)
    }
}
