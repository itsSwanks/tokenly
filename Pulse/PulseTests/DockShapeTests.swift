...
@Suite struct DockShapeTests {
    ...
    /// The Pill style is a flare-less slab: its bounding box is exactly the panel rect, and
    /// the region a flare would have filled — above the panel, on the screen edge — is empty.
    @Test func pillStyleHasNoFlares() {
        let path = DockShape(edge: .right, style: .pill).path(in: rect)
        let box = path.boundingRect
        #expect(box.minX == 0)
        #expect(box.maxX == 78)
        #expect(box.minY == 0)
    ...
    @Test func pillMirrorsOnTheLeftEdge() {
        let right = DockShape(edge: .right, style: .pill).path(in: rect)
        let left = DockShape(edge: .left, style: .pill).path(in: rect)
    ...
    @Test func windowHitTestingExcludesTheTransparentFlareCorners() {
        ...
        let inset = layout.flareInset                            // 20
    ...
    /// The Pill style draws no flares at all, so the whole margin — screen-edge side included —
    /// is click-through.
    @Test func pillWindowHitTestingExcludesTheWholeFlareMargin() {
        let layout = DockLayout(cellCount: 3, edge: .right)
        let bottom = layout.windowSize.height
        #expect(DockShape.contains(windowPoint: CGPoint(x: 77, y: 4), layout: layout, style: .pill) == false)
        #expect(DockShape.contains(windowPoint: CGPoint(x: 77, y: bottom - 4), layout: layout, style: .pill) == false)
        #expect(DockShape.contains(windowPoint: CGPoint(x: 39, y: layout.flareInset + 40), layout: layout, style: .pill))
    }
    ...
    @Test func pillStaysInsideItsRect() {
        let rect = CGRect(x: 0, y: 40, width: 78, height: 300)
        let bounds = DockShape(edge: .right, style: .pill, metrics: .medium).path(in: rect).boundingRect
        #expect(bounds == rect)
    }

    @Test func notchFlaresScaleWithMetrics() {
        let rect = CGRect(x: 0, y: 40, width: 98, height: 300)
        let bounds = DockShape(edge: .right, style: .notch, metrics: DockMetrics(size: .large)).path(in: rect).boundingRect
        #expect(abs(bounds.minY - 15) < 0.01 && abs(bounds.maxY - 365) < 0.01)   // flare radius 25 above and below
    }
}
