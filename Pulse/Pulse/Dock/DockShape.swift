...
/// which grows out of the edge through two concave quarter-circle flares; `.glass` is the same
...
struct DockShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        var p = style == .glass ? glassPath(in: rect) : notchPath(in: rect)
        if edge == .left {
    ...
    /// The same 22 pt inward corners, square against the screen edge, nothing outside `rect`.
    /// Built right-edge-first like `notchPath`, so the inward (rounded) side is the left one.
    private func glassPath(in rect: CGRect) -> Path {
...
extension DockShape {
    ...
    /// only a quarter-circle beside the screen edge, and the Glass slab fills none of it at all.
    ...
    static func contains(windowPoint: CGPoint, layout: DockLayout, style: DockStyle) -> Bool {
        DockShape(edge: layout.edge, style: style)
            .path(in: CGRect(origin: layout.contentOrigin, size: layout.size))
            .contains(windowPoint)
    }
}

/// A rounded rectangle rounded on one vertical side only — the shape shared by the Glass dock
...
/// edge. Kept out of `Shape` so `HandlePanel`'s `CAShapeLayer` mask can use it too.
enum EdgeSlabPath {
