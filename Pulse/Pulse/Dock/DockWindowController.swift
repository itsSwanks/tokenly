...
final class DockWindowController {
    ...
    init(store: UsageStore, prefs: Preferences) {
        self.store = store
        self.prefs = prefs
        let layout = DockLayout(cellCount: prefs.enabledOrdered.count, edge: prefs.edge)
        model.layout = layout
    ...
    /// Push a dock style through the three places that care. The drop shadow is the *window's*
    /// (like the callout's) rather than a SwiftUI `.shadow`, so it can fall outside the hosting
    /// view's bounds and behind the translucent panel instead of on top of the desktop; the
    /// opaque Notch slab sits flush to the bezel and has none.
    private func applyStyle(_ style: DockStyle) {
        if model.style != style { model.style = style }
        interaction.style = style
        panel.hasShadow = style == .glass
    ...
    private func render(on screen: NSScreen) {
        ...
        let layout = DockLayout(cellCount: order.count, edge: prefs.edge)
