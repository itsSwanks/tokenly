...
extension ProviderID {
    /// `.primary` rather than white: on Liquid Glass the surface takes the desktop's brightness,
    /// and a white glyph on light glass measured ~1.3:1. `.primary` is vibrant — dark on a light
    /// surface, light on a dark one — so the mark stays legible on either (spec §18.1).
    @ViewBuilder
    func glyph(size: CGFloat, color: Color = .primary) -> some View {
        switch self {
