...
Claude: the credentials file is read first, and the Keychain item only if that file is missing, undecodable, or expired. Reading it shows a one-time macOS prompt (*"Tokenly wants to access…"*) — choose **Always Allow**; if it is denied, Tokenly stops asking for the rest of the session rather than prompting on every poll.
